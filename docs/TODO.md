...
- [ ] 输入框 `required` 未满足时显示红色边框（全局 form 表单）
- [ ] 导航栏所有元素垂直居中对齐
- [ ] 导航菜单改为卡片式下拉菜单（悬浮卡片样式，非原生下拉）
- [ ] 所有页面统一 padding、margin、border-radius，适配响应式
- [ ] 全站禁止使用 emoji，全部替换为 lucide-react 图标
- [ ] 404 页面设计与实现（中英文）
...
- [ ] `<head>` 标签内容全面完善（charset、viewport、theme-color、robots、canonical 等）
- [ ] OG 标签自动生成（og:title、og:description、og:image、og:url、og:type）
...
- [ ] 后台安全登录页面（独立于前台登录，增加安全校验）
- [ ] 后台导航栏：仅显示后台菜单，不显示前台导航
- [ ] 后台导航包含「返回前台」按钮，点击返回前台首页
