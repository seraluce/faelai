// src/pages/api/user/likes.ts
// 功能：用户点赞列表 API — GET 获取当前用户点赞的文章，支持分页
  ...
  try {
    ...
    const articles = await db
      .select({
        ...
        likedAt: schema.userLikes.createdAt,
      })
      .from(schema.userLikes)
      .innerJoin(schema.articles, eq(schema.userLikes.articleId, schema.articles.id))
    ...
    return new Response(JSON.stringify({ error: '获取点赞列表失败' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
